		...
		preC.setY(e.getY());
			...
			if (!sourisSurQuelleCase(preC).equals(p0)){
				...
				if (!p.listeDeCartes.isEmpty()){
					...
					listeDeplacement = l;
					refX = ColonneOrigine.listeDeCartes.get(sourisSurQuelleRangDeCarte(e.getY(),ColonneOrigine)).posX;
					refY = ColonneOrigine.listeDeCartes.get(sourisSurQuelleRangDeCarte(e.getY(),ColonneOrigine)).posY;
				}
			}
