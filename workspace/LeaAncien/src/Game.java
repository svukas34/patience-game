...
public class Game {
	...
	public Game() {
		for(Carte.Couleur coul : Carte.Couleur.values())
			for (int i = 1; i <= 13; i++)
				defausse.listeDeCartes.add(new Carte(new ImageIcon("Cartes1/"+ i + coul.toString() + ".jpg"), defausse.posX,
	...
	public void InitColonne(TasDeCartes c) {
		for (int i = 1; i < c.listeDeCartes.size(); i++) {
			...
			carte.posY = c.posY + i * 10;/*30*/
		}
	}

	public boolean Win() {
		return (c1.listeDeCartes.isEmpty()&&c2.listeDeCartes.isEmpty()&&c3.listeDeCartes.isEmpty()&&c4.listeDeCartes.isEmpty()&&c5.listeDeCartes.isEmpty()&&c6.listeDeCartes.isEmpty()&&c7.listeDeCartes.isEmpty()&&defausse.listeDeCartes.isEmpty()&&poubelle.listeDeCartes.isEmpty());
	}

	public void ajouterCartePoubelle() {
		if (!defausse.listeDeCartes.isEmpty()) {
			Carte LastCarte = defausse.listeDeCartes.get(defausse.listeDeCartes.size()-1);
		...
		if (l.size() != 0) {
			if (colonne.listeDeCartes.isEmpty()) {
				...
				if ((l.get(0).memeCouleur(LastCarte) == false) && ((LastCarte.valeur) == l.get(0).valeur + 1)) {
					...
					if((l.get(0).memeCouleur(LastCarte))) erreur=1;
					else if(LastCarte.valeur != l.get(0).valeur+1)erreur=2;
					return false;
				}
			}
		} else
	...
	public void ajouterListeColonne(ArrayList<Carte> l, TasDeCartes colonne,TasDeCartes colonneOrigine) {
		if (ajouterListeColonnePossible(l, colonne)) {
			if (colonne.listeDeCartes.isEmpty()) {
	...
	public boolean ajouterCartePilePossible(ArrayList<Carte> l, TasDeCartes pile) {
		if (l.size() == 1) {
			if (pile.listeDeCartes.isEmpty()) {
				...
				Carte LastCarte = pile.listeDeCartes.get(pile.listeDeCartes.size() - 1);
				if ((l.get(0).valeur == ((LastCarte.valeur) + 1)) && (l.get(0).couleur.equals(LastCarte.couleur)))
					return true;
				else {
					if(l.get(0).valeur != ((LastCarte.valeur) + 1))erreur=4;
					if(!l.get(0).couleur.equals(LastCarte.couleur))erreur=5;
					return false;
				}
			}
		} else {
	...
	public void ajouterCartePile(ArrayList<Carte> l, TasDeCartes pile,TasDeCartes colonneOrigine) {
		if (ajouterCartePilePossible(l, pile)) {
